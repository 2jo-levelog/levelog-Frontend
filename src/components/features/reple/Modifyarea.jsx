...
export default function Modifyarea({ data, formatDate, postId, setIsTrue }) {
  const { nickname, createdAt, comment, id } = data;

  const [commentModify, setCommentModify] = useState(false);
  const repleCommentValue = useRef();
  const onModifyCommentHandler = () => {
    if (repleCommentValue.current.value == '') return;
    authInstance
      .put(`/api/posts/comments/${id}`, {
        comment: repleCommentValue.current.value,
      })
      .then(response => {
      ...
      <StProfile>
        <div className="user_conatiner">
          ...
          <StCommentInfo>
            ...
            <span className="comment_date">{formatDate(createdAt)}</span>
          </StCommentInfo>
        </div>
        <StActions>
          <button
        ...
        {commentModify ? (
          <div>
            <StTextarea
              ...
              ref={repleCommentValue}
            />
            <div className="buttons-wrapper">
              <StCancleBtn
                type="button"
                onClick={() => setCommentModify(!commentModify)}
              >
                취소
              </StCancleBtn>
              <StSumitBtn
                type="button"
                onClick={() => onModifyCommentHandler()}
              >
                댓글 작성
              </StSumitBtn>
            </div>
